...
from sklearn.metrics import accuracy_score, plot_confusion_matrix, mean_squared_error, r2_score, roc_auc_score, \
...
scores = cross_val_score(rf, X_train.values, y_train.values, cv=5, scoring='r2')
print('R2 Score CV for RandomForest = {}'.format(np.round(scores.mean(), 2)))

y_pred = rf.predict(X_train.values)
print('RandomForestRegressor results on train data: R2 Score: {}  - MSE Score: {}'.format(r2_score(y_train, y_pred),
                                                                                          mean_squared_error(y_train,
                                                                                                             y_pred)))

y_pred = rf.predict(X_test.values)
print('RandomForestRegressor results on train data: R2 Score: {}  - MSE Score: {}'.format(r2_score(y_test, y_pred),
...
print(X_test)
random_test = {"name": 123.0, "year": 2018, "km_driven": 236598, "fuel": 3.0, "seller_type": 1.0, "transmisssion": 1.0,
               "owner": 2.0, "mileage": 15.00, "engine": 1061, "max_power": 152, "torque": 88.0, "seats": 5.0}
print(random_test)
